...
点击右上角的 **重新连接并刷新所有设备** 按钮时，AYA-Plus 会按 `IP:端口` 去重，对所有已保存的网络设备重新执行 ADB 连接，然后统一读取网络设备、USB 设备和模拟器的最新在线状态。批量执行期间，卡片和表格状态列会逐台显示“等待”“连接中”“核验中”或“失败”，刷新图标同步旋转并提供已完成数量；单台网络设备连接失败不会中断其他设备，刷新完成后会提示在线和离线数量。工具栏的单台 **连接** 按钮也会在操作期间显示 loading，并保持禁用直到最新设备状态核验完成。刷新只更新连接状态和设备信息，不会触发截图。

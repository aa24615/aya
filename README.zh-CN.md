...
设备管理器默认使用卡片视图，同时保留原有表格视图，可通过右上角的卡片和列表按钮随时切换。卡片模式采用每行 5 张的纵向图库布局：截图位于上方并按原始宽高比自适应高度，不裁剪设备画面，右上角使用绿色或灰色 Tag 标识在线、离线状态；截图下方只显示自定义设备名称（没有自定义名称时显示设备型号）和设备 ID。网络设备的 ID 为 `IP:端口`，USB 和模拟器保留原始 ADB ID。超长名称或地址会省略显示，将鼠标移到文字上可查看全文。

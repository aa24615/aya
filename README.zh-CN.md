...
- CSV 导入、导出设备列表
- 设备表格/卡片视图与批量更新截图
- [`aya://` 快捷链接与业务系统集成](docs/URL-Scheme.md)

## 主界面设备列表

主界面左侧使用固定高度的设备卡片，设备较多时可在列表中滚动浏览。卡片第一行优先显示自定义设备名称；未设置设备名称时才显示 ADB 返回的设备型号，不会同时重复展示两者。第二行显示 ADB 设备 ID，其中网络设备显示 `IP:端口`，USB 和模拟器保留原始 ADB ID。主界面左侧卡片不展示备注。搜索框支持按当前显示的设备名称或 IP 地址实时筛选当前已连接设备；搜索只改变左侧列表的显示结果，不会切换或断开当前正在操作的设备。

## 设备卡片与批量截图

设备管理器默认使用卡片视图，同时保留原有表格视图，可通过右上角的卡片和列表按钮随时切换。卡片模式采用每行 5 张的纵向图库布局：截图位于上方并按原始宽高比自适应高度，不裁剪设备画面；设备名称和设备 ID 使用截图下方独立的固定高度区域，因此卡片总高度等于截图高度加文字区域高度，不会互相重叠，每一行都从本行最高卡片之后开始排列。截图右上角使用绿色或灰色 Tag 标识在线、离线状态；文字区域只显示自定义设备名称（没有自定义名称时显示设备型号）和设备 ID。网络设备的 ID 为 `IP:端口`，USB 和模拟器保留原始 ADB ID。超长名称或地址会省略显示，将鼠标移到文字上可查看全文。

选中任意网络设备时，顶部连接栏会自动回填它的 IP 和端口；工具栏的 **重新连接选中设备** 按钮可直接重新连接该设备。点击右上角的 **重新连接并刷新所有设备** 按钮时，AYA-Plus 会按 `IP:端口` 去重，对所有已保存的网络设备重新执行 ADB 连接，然后统一读取网络设备、USB 设备和模拟器的最新在线状态。批量执行期间，卡片和表格状态列会逐台显示“等待”“连接中”“核验中”或“失败”，刷新图标同步旋转并提供已完成数量；单台网络设备连接失败不会中断其他设备，刷新完成后会提示在线和离线数量。工具栏的单台 **连接** 和 **重新连接选中设备** 按钮都会在操作期间显示 loading，并保持禁用直到最新设备状态核验完成。刷新只更新连接状态和设备信息，不会触发截图。

AYA-Plus 会把每台设备最近一次成功截图持久缓存到本机，并在下次启动后继续显示。无线 ADB 设备以 IP 命名，例如 `10.0.0.8.png`；端口变化时仍覆盖同一个文件。USB 和模拟器使用经过文件名安全编码的设备 ID。每台设备最终只保留一张 PNG，新截图会覆盖旧截图；离线设备继续展示最后一次成功缓存，并将卡片缩略图和右侧大图置灰，重新上线后自动恢复彩色。

打开截图页、单击卡片或切换设备时只读取本地缓存，不会调用 ADB 重新截图。主界面截图页和设备管理器右侧都提供 **重新截图** 按钮；点击右上角的 **批量更新截图** 可立即更新全部在线设备。只有这些用户主动操作以及后台定时任务会获取当前设备画面。

应用运行期间会每 1 分钟尝试启动一轮在线设备缓存更新，并限制最多同时截图 3 台。上一轮尚未完成时会跳过本轮，不会重复叠加任务；单台设备失败不会覆盖其旧缓存，也不会中断其他设备。

单击卡片可选择设备并在页面右侧查看大图；双击在线设备可在主界面中切换到该设备。设备列表和右侧截图区域之间的分隔线可以拖动。搜索框在卡片和表格视图中都支持按设备名称、型号、IP/设备 ID、序列号、备注、Android 版本和在线状态查找，也可以组合多个关键词，例如“设备名称 + IP 地址片段”。

截图文件保存在 AYA-Plus 用户数据目录下的 `data/screenshots` 中。为了兼容旧版 AYA 数据，目录名仍沿用 `AYA`；截图可能包含敏感画面，如需清理可在退出应用后删除该 `screenshots` 目录。
...
设备名称和备注保存在 AYA-Plus 本地数据中。网络设备按规范化后的 `IP地址 + 端口` 独立保存，避免多台设备返回相同通用序列号时互相覆盖；USB 和模拟器在有序列号时仍按序列号关联。旧版本已经保存的序列号数据会继续作为兼容回退读取。
...
| --- | --- | --- |
| `设备名称` | 是 | 用户维护的自定义名称 |
| `IP地址` | 网络设备必填 | 网络设备的 IPv4 地址；USB 和模拟器留空 |
| `端口` | 网络设备必填 | ADB 连接端口，与 IP 地址分列导出 |
| `备注` | 是 | 用户维护的备注，可包含逗号和换行 |
| `ID` | USB/模拟器必填 | ADB 设备 ID；网络设备同时保留规范的 `IP:端口` ID 以兼容旧格式 |
| `序列号` | 是 | USB/模拟器用于稳定关联名称和备注；网络设备仍以 IP 和端口唯一识别 |
| `型号` | 是 | ADB 上报的设备型号 |
| `Android 版本` | 是 | Android 系统版本 |
| `SDK 版本` | 是 | Android API/SDK 版本 |
| `状态` | 否 | 导出时的在线或离线状态，仅供查看 |

导出文件固定使用以下列顺序，网络设备的 IP 地址和端口不会再只合并在一个字段中：

```csv
设备名称,IP地址,端口,备注,ID,序列号,型号,Android版本,SDK版本,状态
前台签到机,192.168.1.10,5555,一楼大厅,192.168.1.10:5555,SN001,Pixel 8,14,34,在线
```
...
- AYA-Plus 以前导出的八列完整备份格式仍然兼容；这类文件继续使用 `ID` 列恢复全部已保存字段。
- 支持中文或英文表头，不区分大小写、空格、下划线和连字符。
- 网络设备严格按规范化后的 `IP地址 + 端口` 唯一匹配；例如端口 `05555` 和 `5555` 视为同一端口。
- 导入的 IP 地址和端口已经存在时，只覆盖原设备的“设备名称”和“备注”，不会再新增一条相同设备记录。
- 同一个 CSV 内重复出现相同 IP 地址和端口时，只保留最后一行，并且只执行一次 ADB 连接。
- 同时提供 IP、端口和 `ID` 时，以拆分后的 IP 地址和端口为准；USB 和模拟器继续按 `ID` 或序列号关联。
...
完整的操作、参数、编码和系统接入说明请查看 [AYA-Plus URL Scheme 使用文档](docs/URL-Scheme.md)。

...
点击右上角的 **重新连接并刷新所有设备** 按钮时，AYA-Plus 会按 `IP:端口` 去重，对所有已保存的网络设备重新执行 ADB 连接，然后统一读取网络设备、USB 设备和模拟器的最新在线状态。单台网络设备连接失败不会中断其他设备；存在已保存的网络设备时，刷新完成后会提示在线和离线数量。刷新只更新连接状态和设备信息，不会触发截图。

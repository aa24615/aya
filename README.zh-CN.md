...
AYA-Plus 会把每台设备最近一次成功截图持久缓存到本机，并在下次启动后继续显示。无线 ADB 设备以 IP 命名，例如 `10.0.0.8.png`；端口变化时仍覆盖同一个文件。USB 和模拟器使用经过文件名安全编码的设备 ID。每台设备最终只保留一张 PNG，新截图会覆盖旧截图，离线设备也会继续展示最后一次成功缓存。
